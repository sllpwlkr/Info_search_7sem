...
static bool parse_token_from_tokens_tsv_line(const std::string& line, std::string& token_out) {
    // Формат tokens.tsv: doc_id \t pos \t token
    // Токен — всё после второй табуляции.
    size_t p1 = line.find('\t');
...
int main(int argc, char** argv) {
    // Значения по умолчанию (автозапуск без параметров)
    ...
    // Опциональные параметры: zipf [tokens.tsv] [out_dir] [topN]
    ...
    // 1) Сортировка лексикографически
    std::sort(terms.begin(), terms.end());

    // 2) Подсчёт частот одним проходом
    ...
    // освобождаем память, terms больше не нужен
    std::vector<std::string>().swap(terms);

    // 3) Сортировка по частоте убыванию (для рангов)
    std::sort(freqs.begin(), freqs.end(), [](const TermFreq& a, const TermFreq& b) {
        if (a.freq != b.freq) return a.freq > b.freq;
        return a.term < b.term;
    });

    const uint32_t C = freqs[0].freq;   // константа Ципфа = частота термина ранга 1
    const size_t V = freqs.size();      // размер словаря

    std::cout << "Vocabulary (unique terms): " << V << "\n";
    std::cout << "C (freq at rank 1): " << C << "\n";

    // Файл rank-freq для графика (лог-лог и линия Ципфа)
    // columns: rank freq log10_rank log10_freq zipf_freq
