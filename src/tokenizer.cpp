...
struct Stats {
    ...
    uint64_t total_token_chars = 0;  // в wchar_t-символах
    uint64_t total_bytes_text = 0;   // байты clean_text (UTF-8)
};
...
// clean_text: строка JSON с минимальными экранированиями
...
// Токенизация:
// - буквы/цифры входят в токен
// - дефис входит в токен, только если он между двумя алфавитно-цифровыми
// - приводим к нижнему регистру
// - фильтр: токены длиной < 3 выкидываем, но числа оставляем всегда
...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        std::cerr << "Usage: tokenizer <input.jsonl> <output_dir>\n";
        std::cerr << "Example: tokenizer data/corpus.jsonl data/tokens\n";
        return 1;
    }
    ...
    // Один файл со всеми токенами
    std::ofstream tokens_out(out_dir / "tokens.tsv", std::ios::binary);
    if (!tokens_out) {
        std::cerr << "Cannot open tokens.tsv\n";
        return 1;
    }

    // Индекс документов: doc_id -> byte_offset + token_count
    std::ofstream docs_idx(out_dir / "docs.idx", std::ios::binary);
    if (!docs_idx) {
        std::cerr << "Cannot open docs.idx\n";
        return 1;
    }
    ...
    while (std::getline(in, line)) {
        ...
        // Где в tokens.tsv начинается этот документ
        ...
        // docs.idx: doc_id \t byte_offset \t token_count
    ...
    std::cout << "Docs processed: " << docs << "\n";
    std::cout << "Total tokens: " << stats.total_tokens << "\n";
    std::cout << "Avg token length (chars): " << avg_len << "\n";
    std::cout << "Input bytes (clean_text): " << stats.total_bytes_text << " (" << kb << " KB)\n";
    std::cout << "Time: " << sec << " sec\n";
    std::cout << "Speed: " << kb_per_sec << " KB/s\n";
    std::cout << "Speed: " << us_per_kb << " us/KB\n";
    std::cout << "Speed: " << tok_per_sec << " tokens/s\n";
    std::cout << "Tokens saved to: " << (out_dir / "tokens.tsv") << "\n";
    std::cout << "Docs index saved to: " << (out_dir / "docs.idx") << "\n";
