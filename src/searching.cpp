...
struct InvertedIndex {
    ...
    std::vector<std::string> doc_ids; // Список идентификаторов документов как строки
};

// Загрузка прямого индекса из бинарного файла
void load_direct_index(const std::string &filename, std::vector<DirectIndex> &direct_index) {
    ...
    while (infile) {
        ...
        // Строковый doc_id, изменим на чтение из файла
...
// Загрузка обратного индекса из бинарного файла
    ...
    for (size_t i = 0; i < query.size(); ++i) {
        ...
        if (c == '"' && !in_quotes) {
            ...
            if (i + 1 < query.size() && query[i + 1] == c) {
                tokens.push_back(std::string(1, c) + c); // && или ||
                ++i;
            } else {
...
// Выполнение логических операций AND, OR, NOT
...
// Функция для выполнения булевого поиска
