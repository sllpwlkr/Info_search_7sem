...
class WikiCrawler:
    ...
    def process_page(self, page):
        ...
        try:
            ...
            if word_count < self.min_words:
                ...
                # Если статья уже есть в базе и стала короткой - удаляем
                if existing:
            ...
            # Вычисляем хэш для определения изменений
            content_hash = self.db.compute_hash(clean_text)

            # Проверяем, изменился ли контент
            if existing and existing.get('content_hash') == content_hash:
                # Контент не изменился, только обновляем время
    ...
    def continue_crawling(self):
        """Продолжение обхода с места остановки"""
        # Можно реализовать логику продолжения
        # Пока просто начинаем с начала
        logger.info("Начало новой сессии обхода")
        self.crawl_category(self.start_category_obj)

    def run(self):
        """Основной метод запуска робота"""
        logger.info("=" * 60)
...
class BreCrawler:
    """
    Crawler для БРЭ на old.bigenc.ru, собирает статьи из sitemap.xml.
    """
    ...
    def _parse_sitemap_urls(self, sitemap_xml: str) -> list[str]:
        """
        Возвращает список <loc> из sitemap.
        Поддерживает и sitemapindex, и urlset.
        """
        urls = []
        try:
    ...
    def extract_title(self, html: str) -> str:
        """
        Надёжно вытаскивает название статьи из HTML БРЭ:
        1) og:title
        2) <h1>
        3) <title>
        4) <meta name="title">
        ...
        try:
            ...
            if soup.title:
                ...
                if t:
                    # иногда title вида "Слон — БРЭ" / "Слон | bigenc"
    ...
    def process_url(self, url: str) -> bool:
        try:
            ...
            if word_count < self.min_words:
                ...
                self.too_short += 1
                # если была в базе — удалим, чтобы не засорять корпус
                if existing:
                    self.db.collection.delete_one({"_id": existing["_id"]})
                return False
    ...
    def run(self):
        ...
        try:
            ...
            # если index оказался сразу urlset — значит это sitemap со статьями
            # тогда sitemaps = список статей, а не список sitemap-файлов
            # распознаём по расширению/паттерну
