...
static std::wstring stem_ru_porter(const std::wstring& token) {
    ...
    // В Snowball обычно 'ё' заменяют на 'е'. Это уменьшает шум.
    for (auto& ch : w) if (ch == L'ё') ch = L'е';

    // Найти RV: позиция после первой гласной
...
int main(int argc, char** argv) {
    ...
    while (std::getline(in, line)) {
        ...
        try {
            ...
            // если не распарсили UTF-8 — пишем как есть
            out << doc_id << "\t" << pos << "\t" << token << "\n";
            total_written++;
            continue;
        }

        // 1) ВЫКИДЫВАЕМ чисто числовые токены полностью
        if (is_all_digits_ws(wtok)) {
            dropped_numeric++;
            continue; // НЕ пишем строку
        }

        std::wstring stemmed = wtok;

        // 2) Не стеммить слова длиной <= 3: только lower
        if (wtok.size() <= 3) {
            stemmed = to_lower_ws(wtok);
        }
        // 3) Кириллица: стемминг
        else if (contains_cyrillic(wtok)) {
            stemmed = stem_ru_porter(wtok);
        }
        // 4) Латиница/прочее: lower
        else {
            stemmed = to_lower_ws(wtok);
        }
    ...
    std::cout << "Tokens read    : " << total_read << "\n";
    std::cout << "Tokens written : " << total_written << "\n";
    std::cout << "Dropped numeric: " << dropped_numeric << "\n";
    std::cout << "Changed tokens : " << changed << "\n";
    return 0;
}
