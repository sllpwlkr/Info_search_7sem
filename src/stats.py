...
# Mongo опционально: если нет pymongo — просто пропустим секцию raw
try:
...
def analyze_corpus_jsonl(corpus_path: str) -> dict:
    """
    corpus.jsonl создаётся export_corpus.py:
      doc_id, normalized_url, source_name, title, clean_text
    """
    t0 = time.time()
    ...
    # грубая оценка "слов": по пробелам (на русском для отчёта обычно достаточно)
    total_words_est = 0
...
def analyze_tokens(tokens_path: str, docs_idx_path: str, top_n: int = 30) -> dict:
    ...
    tokens.tsv format:
      doc_id \t pos \t token
    docs.idx format:
      doc_id \t byte_offset \t token_count
    """
    # 1) по docs.idx: сколько документов и токенов в каждом
    doc_count = 0
    tokens_per_doc = []
    ...
    # 2) по tokens.tsv: общее число токенов, средняя длина, top-n частот
    # Чтобы не жрать память на огромный vocab, держим частоты в dict (python это ок)
    t0 = time.time()
    total_tokens = 0
    total_token_chars = 0
    total_bytes_read = 0
    ...
    with open(tokens_path, "rb") as fb:
        for raw_line in fb:
            ...
            # частоты токенов (для Ципфа потом пригодится)
...
def analyze_mongo_raw(config_path: str) -> Optional[dict]:
    """
    Опциональная секция для ЛР-1: сырой html + сверка данных.
    Выполняется только если установлен pymongo и доступна БД.
    """
    ...
    # только документы с raw_html/clean_text
    cur = col.find(
        {},
...
def main():
    ...
    if args.config:
        try:
            ...
            # не падаем, просто фиксируем ошибку
